    ...
    <div ref={cardRef} className="w-full relative"
      ...
      {hasOpenHouse && (
        <div className="absolute top-[calc(env(safe-area-inset-top,12px)+70px)] left-4 z-10">
          <span className="bg-open-amber/20 text-open-amber text-[10px] font-bold uppercase px-2.5 py-1 rounded-full backdrop-blur-sm">Open House</span>
        </div>
      )}

      {/* Right sidebar */}
      <div className="absolute right-3 bottom-[22%] z-10 flex flex-col items-center gap-5">
        {/* Agent avatar — tap to follow/unfollow */}
        <motion.button whileTap={!isPreview ? { scale: 0.85 } : undefined}
          ...
          {!following && !isPreview && (
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-5 h-5 rounded-full bg-tangerine flex items-center justify-center">
              <UserPlus size={10} className="text-white" />
            </div>
          )}
          {following && (
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-5 h-5 rounded-full bg-sold-green flex items-center justify-center">

...
export interface MapShape {
  ...
  expandScale: number
  /** State shapes only — fraction of `size` to add to the EXPAND
   *  call's (cx, cy) so the path's pole of inaccessibility lands
   *  at the container center instead of the bbox midpoint. Several
   *  states (notably Florida) have bbox midpoints that fall outside
   *  the silhouette, so bbox-centered expand leaves cardBg slivers
   *  exposed. The pole offset shifts the silhouette so its most-
...
/* ── State shapes ──────────────────────────────────────────────
   ...
   `./stateShapes.ts` as normalized [0,1]×[0,1] M/L/Z paths. The
   runtime generator below scales/translates those into the same
   `(cx, cy, size)` coordinate system the geometric shapes use,
   so the same morph animation works on expand/dismiss. */
import { STATE_SHAPES, type StateShapeData } from './stateShapes'

const stateCoordRe = /(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)/g

/** Build a `ShapePath` from a state's unit-space `d`. The unit-d
 *  uses only M/L/Z so every consecutive pair of numbers is an
 *  (x, y) coordinate — a single regex pass scales+translates them
 *  into the renderer's coord space. Bbox-centered: unit (0.5, 0.5)
 *  lands at (cx, cy). The silhouette is the state's largest
 *  contiguous land mass, so the bbox center is reliably interior
 *  and the container projection at high expandScale stays inside
 *  the silhouette regardless of viewport aspect. */
function stateShapePath(unitD: string): ShapePath {
  return (cx, cy, size) => {
    const x0 = cx - size / 2
    const y0 = cy - size / 2
    const transformed = unitD.replace(stateCoordRe, (_, xs: string, ys: string) => {
      const x = (x0 + parseFloat(xs) * size).toFixed(2)
      const y = (y0 + parseFloat(ys) * size).toFixed(2)
      return `${x} ${y}`
    })
...
/** Generate a registry entry for a single state. ID prefix lets
 *  the StyleTab + audit distinguish state picks from the geometric
 *  shapes without a separate field. */
function buildStateShape(s: StateShapeData): MapShape {
  // PEEK uses bbox-centered (silhouette stays inside the slot,
  // vertically centered, similar scale across states).
  //
  // EXPAND uses pole-of-inaccessibility anchoring — the silhouette
  // shifts so the polygon's most-interior point lands at the
  // container center. Critical for asymmetric states like Florida
  // (bbox midpoint = Gulf of Mexico, NOT inside FL): without the
  // shift, the container projects to that empty region and the
  // silhouette doesn't cover.
  //
  // expandScale per-state derives from the pole's clearance —
  // we need the container's projection (≤ 1/expandScale per axis)
  // to fit inside the inscribed disk of radius `clearance` around
  // the pole. Worst-case (square viewport): half-diagonal of
  // projection = √2/(2·expandScale), which must be ≤ clearance.
  // → expandScale ≥ √2/(2·clearance) ≈ 0.707/clearance.
  // We use 1.0/clearance for healthy margin.
  const expandScale = Math.max(2.8, 1.0 / s.clearance)
  const expandAnchorOffset = { x: 0.5 - s.poleX, y: 0.5 - s.poleY }
  return {
    id: `state_${s.code}`,
    name: s.name,
    vibe: `${s.code} state outline`,
    path: stateShapePath(s.d),
    peekScale: 0.96,
    expandScale,
    expandAnchorOffset,
  }
...
/** Just the state shapes — used by the StyleTab dropdown without
 *  iterating the full registry. */
export { STATE_SHAPES, type StateShapeData } from './stateShapes'
...
/** True when the shape id refers to a US state outline. Used by
 *  the StyleTab + audit + ShapeCard render to branch behavior
 *  (different picker UI, different "Pro" messaging). */
export function isStateShape(id: string): boolean {
  return id.startsWith('state_')
}
